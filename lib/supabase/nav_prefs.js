...
//
// This table is SUITE-WIDE, not a comms table: sidebar curation is a property of
// the user, not of the comms domain. It lives in the shared `public` schema
...
//
// createClient() defaults to the `comms` schema, so every call here re-scopes to
// `public` explicitly.
//
...
export function normalizeNavPrefs(row) {
  ...
  return {
    ...
    hidden: Array.isArray(row.hidden)
      ? row.hidden.filter((t) => typeof t === "string")
      : [],
  };
}

// This user's row for the project, or an empty-prefs stand-in when there is
// none yet — the caller treats both as "nothing hidden".
...
// (product, surface, project_id, user_id) is the upsert target; the table has
// no soft delete (it was dropped so this upsert could work at all), so clearing
// a preference means storing an empty `hidden` array.
